...
cpu_rsu_tick(void) {
	...
	union attributes tile_attr = { bus_read_byte(BG_ATTR + (tl.y * 16 + tl.x)) };
	u16 bg_tiles_address = bus_read_word(BG_TILES);
	u16 tile_row = bus_read_word(bg_tiles_address + 16 * tile_index + px.y * 2);
	tile_row = (tile_row & 0xff << 8) | (tile_row >> 8);
	u8 pixel = tile_row >> (px.x * 2) & 0b11;
	u8 color = bus_read_byte(BG_PALS + tile_attr.palette * 2 + pixel);
	crt_electron_gun_shoot(colors[color]);
	px.x = px.x +     1;
	px.y = px.y + !px.x;
	if (!px.y) {
